...
impl Socket {
    ...
    {
        tracing::warn!("creating new socket...");
        ...
        let last_alive_clone = last_alive.clone();
        let timeout = config.timeout.clone();
        handle.spawn(async move {
            socket_heartbeat(
                sink_clone,
                config,
                hearbeat_abort_receiver,
                last_alive_clone,
            )
            .await
        });
        ...
        handle.spawn(async move {
            loop {
                ...
                sleep.await;
                // tracing::info!("slept for {timeout:?}");
                let elapsed_since_last_alive = la.lock().await.elapsed();
                if elapsed_since_last_alive > timeout {
                    stream_clone
                        .sender
...
async fn handle_heartbeat_sleep_elapsed(
    ...
    if elapsed_since_last_alive > config.timeout {
        tracing::info!("closing connection due to network issues");
        let _ = sink
